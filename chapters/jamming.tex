...
\begin{equation}
\sigma_x^2 = \sum_{j=1}^{N_b} \frac{<X_j^2>-<X_j>^2}{N_b-1}
\end{equation}
where $N_b$ is the number of blocks, determined empirically according to the shear rate.

\myparagraph{Fitting parameters}

Errors on the parameters returned by the Levenberg-Marquadt algorithm are determined by observing how these parameters vary when including noise in the input parameters.
...
\begin{equation}
\eta_p^s(\phi,\dot{\gamma}) = A(\phi_J-\phi +cE(\phi,\dot{\gamma})^{-h_p/2q_p})^{-\beta}
...
Another, more time-consuming, solution is of course to apply the Levenberg-Marquadt algorithm (part \ref{levenberg_marquadt}) on equation \ref{phieff_soft2hard_check} with $A$ and $c$ as free parameters.

\section{Rheology}

The shearing rheology of athermal frictionless soft-core disks has been thoroughly studied in \cite{vaagberg2017shear,PRL112.208303} and gives hints on what to expect of the shearing rheology of spheroids in our model near the jamming transition.\\

Vågberg \textit{et al.} showed that upon increasing the packing fraction, disks discontinuously transitioned from Bagnoldian rheology
\begin{equation}
...
These two phases differ in that the particles tend to cluster together in the Newtonian phase, with force chains that percolate throughout the system, while in the Bagnoldian phase particles have fewer instantaneous contacts. We must then expect a sharp increase in the number of contacts at the transition between these regimes.\\

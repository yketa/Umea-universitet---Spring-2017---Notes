...
Renormalisation group theory suggests that, close to $(\delta\phi\rightarrow0,\dot{\gamma}\rightarrow0)$, there exists $y_{\delta\phi}$ and $y_{\dot{\gamma}}$ such that the correlation length transforms under a change of length scale with a factor $l$ as
\begin{align*}
...
We will assume that variables which vanish at jamming are somehow linked to the free energy density of the system. Therefore, renormalisation group theory here suggests the following assumption on the scaling of a vanishing variable $\mathcal{O}$ close to the jamming point, \textit{i.e.} $(\delta\phi\rightarrow0,\dot{\gamma}\rightarrow0)$, for a change of length scale with a factor $l$ 
\begin{equation}
...
\begin{align*}
p(\delta\phi,\dot{\gamma}) \sim \left(\frac{\delta\phi}{b}\right)^{-y_{\mathcal{O}}} g_0\left(b,\dot{\gamma}\left(\frac{\delta\phi}{b}\right)^{-\nu z}\right)
\end{align*}
...
\end{equation}
with $y_{\mathcal{O}}$ being then the critical exponent associated to the vanishing of the variable $\mathcal{O}$ at jamming.

\subsection{Pressure}

We introduce the function $g_1$ such that equation \ref{scaling_assumption_general} applied to the pressure $p$ can be written
\begin{align*}

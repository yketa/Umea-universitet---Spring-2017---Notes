...
\end{align*}
and use the Levenberg-Marquardt algorithm \cite{marquardt1963algorithm,press2015numerical} with $\phi_J$, $z_c$, $q_{\mathcal{O}}$, $h_{\mahtcal{O}}$ and $a_{\mathcal{O},0}$ through $a_{\mathcal{O},5}$ as free parameters to the corresponding variable fitting.\\

This then allows us to determine the relevant critical parameters at the jamming transition.\\

We have shown that $z_c$ may be determined both by the observation of the relaxation process according to equation \ref{tau_dz_scaling} and by the analysis of shearing simulations according to equation \ref{fitting_function_contact_number}.\\

However, **** impossible to use it in our simulations because of [???] ****
...
Vågberg \textit{et al.} showed that upon increasing the packing fraction, disks transitionned from Bagnoldian rheology
\begin{equation}
p \sim \dot{\gamma}^2 \Leftrightarrow \frac{\partial}{\partial\dot{\gamma}} B_p = 0,~ B_p \equiv p/\dot{\gamma}^2
\end{equation}
to Newtonian rheology
\begin{equation}
p \sim \dot{\gamma} \Leftrightarrow \frac{\partial}{\partial\dot{\gamma}} \eta_p = 0,~ \eta_p \equiv p/\dot{\gamma}
\end{equation}
